...
public final class FailoverNode {
    ...
    public Integer getItemByExecutionFailoverPath(final String path) {
        ...
        return Integer.parseInt(path.substring(JobNodePath.getNodeFullPath(jobName, ExecutionNode.ROOT).length() + 1, path.lastIndexOf(FailoverNode.FAILOVER) - 1));
    }
    
    private boolean isFailoverPath(final String path) {
        return path.startsWith(JobNodePath.getNodeFullPath(jobName, ExecutionNode.ROOT)) && path.endsWith(FailoverNode.FAILOVER);
    }
}
