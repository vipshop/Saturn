...
public class ContainerServiceImpl implements ContainerService {
    ...
    public void saveOrUpdateContainerToken(ContainerToken containerToken) throws SaturnJobConsoleException {
        ...
        try {
            // Update scale job's shardingItemParameters. Disable job, sleep 1s, update shardingItemParameters, sleep 1s, enable job
    ...
    private String changeTypeOfContainerStatus(ContainerToken containerToken, String task) {
        ...
        try {
            ...
            errorMessage = "<font color='red'>get task status error, message is: " + StringEscapeUtils.escapeHtml4(e.getMessage()) + "</font>";
        } finally {
            ...
            return String.format("当前实例总数：%s<br/><br/>" +
                            "健康实例数：%s<br/>" +
                            "非健康实例数：%s<br/><br/>" +
                            "staged实例数：%s<br/>" +
                            "running实例数：%s<br/>" +
    ...
    private String changeTypeOfContainerConfig(ContainerConfig containerConfig) {
        ...
        return String.format("配置的实例数：%s<br/>" +
                        "CPU：%s核<br/>" +
                        "内存：%sM<br/>" +
                        "镜像：%s<br/>" +
                        "约束标识：%s<br/>" +
                        ...
                        "容器与宿主机的目录映射：%s<br/>" +
                        "CMD：%s",
                containerConfig.getInstances(), containerConfig.getCpus(), containerConfig.getMem(), containerConfig.getImage(), constraints, env, privileged, force_pull_image, volumes, cmd);
    }
    ...
    private String getContainerScaleJobShardingItemParameters(ContainerToken containerToken, String taskId, Integer instances) throws SaturnJobConsoleException {
