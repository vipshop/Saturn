...
public class ZKDBDiffController extends AbstractController {
	...
	/**
	 * 按照zk集群，对比zk和db的数据差异。<br>
	 * 如果console和zk集群不是在同一机房，则会将调用zk所属机房的console rest api去进行对比，如果对比失败，则继续在本机房进行对比；
	 *
	 * @param request
	 * @param zkCluster
	 * @return
	...
	private List<JobDiffInfo> relayDiffIfPossible(String zkCluster) throws SaturnJobConsoleException {
		try {
			List<JobDiffInfo> result = relayDiff(zkCluster);
			if (result == null) {
	...
	private String buildTargetUrl(String zkCluster, String consoleDomain) throws SaturnJobConsoleException {
		if (StringUtils.isBlank(consoleDomain)) {
			throw new SaturnJobConsoleException("no console domain defined in the system config!");
		}
