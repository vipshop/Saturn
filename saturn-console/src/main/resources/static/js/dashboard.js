...
function renderAbnormalJob() {
	...
	$.post("dashboard/unnormalJob", {}, function (data) {
        ...
        for (var i in data) {
        	trContent += Mustache.render(unnormalJobTmp,{cause:causeMap[data[i].cause],degree:degreeMap[data[i].degree],jobDegree:degreeMap[data[i].jobDegree], jobName:data[i].jobName, domainName:data[i].domainName, nns:data[i].nns+"&", nextFireTime:new Date(data[i].nextFireTime).format("yyyy-MM-dd HH:mm:ss")});
        }
...
function renderVersionDomainNumber() {
	$.post("dashboard/versionDomainNumber", {}, function (data) {
		var rankArray = [];
		if(data) {
...
function renderVersionExecutorNumber() {
	$.post("dashboard/versionExecutorNumber", {}, function (data) {
		var rankArray = [];
		if(data) {
            for (var version in data) {
                var element = []
